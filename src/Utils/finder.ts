    ...
    static async finder(model: any, args: any, query: QueryBuilder<any>): Promise<any> {
        ...
        let inquiry = await query
            ...
            .modify(qb => {
                ...
                if (q) {
                    const columnValueMatch = q.match(/(\w+):(.*)/);
                    console.log('subject: ', q);
                    if (columnValueMatch) {
                        const column = columnValueMatch[1];
                        const value = columnValueMatch[2];
                
                        if (columns.includes(column)) {
                            if (column === 'subject') {
                            
                                const tsQuery = `${value}:*`;
                                qb.whereRaw('subject @@ to_tsquery(?)', [tsQuery]);
                            } else if (column === 'regsir') {
                                const tsQuery = `${value}:*`;
                                qb.whereRaw('regsir @@ to_tsquery(?)', [tsQuery]);
                            } else {
                                console.log('Unsupported column: ', column);
                                // Handle unsupported column if needed
                            }
                        } else {
                            console.log('Column not in columns array: ', column);
                            // Handle case where specified column is not allowed
                        }
                    } else {
                        console.log('Invalid q parameter format: ', q);
                        // Handle case where q parameter format is invalid
                    }
                } 
